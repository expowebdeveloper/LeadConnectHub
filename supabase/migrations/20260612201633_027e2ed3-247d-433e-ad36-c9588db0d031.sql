...
-- Bump home_value_max default in stored weights
UPDATE public.scoring_weights
SET weights = weights || jsonb_build_object('home_value_max', 60),
    updated_at = now()
WHERE id = 1;
...
BEGIN
  ...
  hv_max     := coalesce((w->>'home_value_max')::int, 60);
  ...
  -- Property: presence of year_built/sqft is a small bump; dwelling value dominates
  v_property := 0;
  ...
  IF p_dwelling_value IS NOT NULL THEN
    v_dwelling := CASE
      WHEN p_dwelling_value >= 1000000 THEN hv_max
      WHEN p_dwelling_value >=  750000 THEN (hv_max * 90) / 100
      WHEN p_dwelling_value >=  500000 THEN (hv_max * 75) / 100
      WHEN p_dwelling_value >=  350000 THEN (hv_max * 55) / 100
      WHEN p_dwelling_value >=  250000 THEN (hv_max * 40) / 100
      WHEN p_dwelling_value >=  150000 THEN (hv_max * 25) / 100
      ELSE                                   (hv_max * 10) / 100
    END;
    v_property := v_property + v_dwelling;
  END IF;
  ...
  RETURN QUERY SELECT auto_total, home_total, comp, tier,
    jsonb_build_object(
      ...
      'bundling', v_bundling, 'property', v_property, 'dwelling', v_dwelling,
      'auto_bucket', auto_bucket, 'home_bucket', home_bucket,
      'winback_bonus', wb_bonus
    );
END;
$function$;

-- Refresh existing scores for both tables (fires recalc trigger)
UPDATE public.leads      SET updated_at = updated_at WHERE archived_at IS NULL;
UPDATE public.list_leads SET updated_at = updated_at WHERE archived_at IS NULL;
