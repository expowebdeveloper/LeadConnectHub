...
BEGIN
  ...
  IF auto_norm IS NULL THEN
    ...
    ELSIF inferred_auto THEN
      v_auto_carrier := LEAST(pts_unk, 6); auto_bucket := 'missing';
    ELSE
  ...
  IF home_norm IS NULL THEN
    ...
    ELSIF inferred_home THEN
      v_home_carrier := LEAST(pts_unk, 6); home_bucket := 'missing';
    ELSE
  ...
  v_vehicles := LEAST(coalesce(p_num_vehicles, 0) * coalesce((w->>'vehicle_per_unit')::int,10), coalesce((w->>'vehicle_cap')::int,50));
...
SET weights = jsonb_strip_nulls(coalesce(weights, '{}'::jsonb) || jsonb_build_object(
  'vehicle_per_unit', 10,
  'vehicle_cap', 50,
  'tier_winback_base', 2,
