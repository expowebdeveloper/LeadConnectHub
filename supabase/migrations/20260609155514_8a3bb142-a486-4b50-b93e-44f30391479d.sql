...
DROP FUNCTION IF EXISTS public.calc_lead_score(uuid, text, text, text, integer, text, text, date, timestamp with time zone, text, text, text[], integer, integer, numeric, boolean, boolean);
...
BEGIN
  ...
  ELSIF is_winback THEN
    tier := 'A'; comp := comp + coalesce((w->>'tier_winback_bonus')::int, 35);
  ELSIF is_aged THEN
