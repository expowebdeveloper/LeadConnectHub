...
SET weights = weights
  || jsonb_build_object(
    ...
    'carrier_pts_none',     15,
    'carriers_cheap',    to_jsonb(ARRAY['Geico','Progressive','Root','Esurance','The General','Direct Auto','Dairyland','Bristol West','Lemonade']),
    'carriers_standard', to_jsonb(ARRAY['State Farm','Allstate','USAA','Nationwide','Liberty Mutual','Safeco','Travelers','National General','Kemper','Auto Club']),
    'carriers_premium',  to_jsonb(ARRAY['Farmers','AAA','American Family','Auto-Owners','The Hartford','Erie','Plymouth Rock','Commerce','MetLife','Mercury'])
  )
WHERE id = 1;
...
DECLARE
  ...
  pts_cheap int; pts_std int; pts_prem int; pts_unk int; pts_none int;
  cheap_l text[]; std_l text[]; prem_l text[];
...
UPDATE public.leads      SET scored_at = scored_at;
UPDATE public.list_leads SET scored_at = scored_at;
