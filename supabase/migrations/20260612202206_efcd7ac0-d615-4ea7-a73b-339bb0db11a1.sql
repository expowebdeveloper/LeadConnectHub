...
SET weights = weights || jsonb_build_object('home_value_max', 80),
    updated_at = now()
WHERE id = 1;
...
BEGIN
  ...
  h_value_max   := coalesce((w->>'home_value_max')::int, 80);
  ...
  -- Home age: full pts <=10y, linear to 0 at 80y. Missing => 0 (no free credit).
  ...
  -- Dwelling value: 0 when missing, tiered when present (dominant home factor).
  v_home_value := 0;
  IF p_dwelling_value IS NOT NULL AND p_dwelling_value > 0 THEN
    v_home_value := CASE
      WHEN p_dwelling_value >= 1000000 THEN h_value_max
      WHEN p_dwelling_value >=  750000 THEN (h_value_max * 90) / 100
      WHEN p_dwelling_value >=  500000 THEN (h_value_max * 75) / 100
      WHEN p_dwelling_value >=  350000 THEN (h_value_max * 55) / 100
      WHEN p_dwelling_value >=  250000 THEN (h_value_max * 40) / 100
      WHEN p_dwelling_value >=  150000 THEN (h_value_max * 25) / 100
      ELSE                                   (h_value_max * 10) / 100
    END;
  END IF;

  -- Roof recency: full <=5y, linear to 0 at 30y. Missing => 0.
  ...
  ELSIF ct ~ '(frame|wood)' THEN v_home_construction := h_frame;
  ELSE v_home_construction := 0;
  END IF;

  fz := lower(coalesce(p_flood_zone, ''));
  IF fz ~ '^(a|v)' THEN v_home_flood := h_flood_high;
  ELSIF fz <> '' THEN v_home_flood := h_flood_low;
  ELSE v_home_flood := 0;
  END IF;
