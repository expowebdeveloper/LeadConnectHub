-- Make num_vehicles the dominant scoring factor; reduce missing-info penalty;
-- soften aged penalty; remove 100-point ceilings so high-vehicle requotes
-- naturally outrank low-vehicle winbacks.

CREATE OR REPLACE FUNCTION public.calc_lead_score(p_vendor_id uuid, p_phone text, p_email text, p_housing_status text, p_num_vehicles integer, p_auto_carrier text, p_home_carrier text, p_date_of_birth date, p_created_at timestamp with time zone, p_dispo text, p_home_dispo text, p_lead_types text[], p_year_built integer, p_square_feet integer, p_dwelling_value numeric, p_is_list_source boolean, p_is_aged_source boolean, p_list_type text DEFAULT NULL::text)
...
BEGIN
  ...
  -- Neutral on age when missing — do not punish
  ...
  -- Carrier: missing info is neutral (full unknown points), not punitive
  IF auto_norm IS NULL THEN
    IF explicit_auto THEN
      v_auto_carrier := pts_none; auto_bucket := 'none';
    ELSIF inferred_auto THEN
      v_auto_carrier := pts_unk; auto_bucket := 'missing';
    ELSE
      v_auto_carrier := 0; auto_bucket := 'n/a';
    END IF;
  ...
  IF home_norm IS NULL THEN
    ...
    ELSIF inferred_home THEN
      v_home_carrier := pts_unk; home_bucket := 'missing';
    ELSE
      v_home_carrier := 0; home_bucket := 'n/a';
    END IF;
  ...
  -- DOMINANT FACTOR: vehicle count. 15 pts per vehicle, no practical cap (150 for 10 cars).
  ...
  -- No 100-cap on subscores: let high-vehicle leads dominate ranking.
  ...
  -- Winback bonus stays modest so it can't overpower 2-3 more vehicles
  ...
  ELSIF is_winback THEN
    ...
    comp := comp + wb_bonus;
  ELSIF is_aged THEN
    -- Soft penalty: aged still ranks below same-vehicle live/winback but vehicle count wins
  ...
  -- Allow composite to exceed 100 so vehicle dominance is preserved in sort order
...
-- Update saved weights to match new dominant-vehicle config
...
-- Recompute scores for all open pool leads
