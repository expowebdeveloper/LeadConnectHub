    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          ...
          await sendLovableEmail(
            {
              ...
              label: emailPayload.label,
              idempotency_key: emailPayload.idempotency_key,
              unsubscribe_token: emailPayload.unsubscribe_token,
          ...
          console.error('Immediate email send failed; queued retry remains available', {
            error: sendError,
            templateName,
            recipient_redacted: redactEmail(effectiveRecipient),
          })
        ...
        return Response.json({ success: true, queued: true, flushed })
      },
    },
  },
})
